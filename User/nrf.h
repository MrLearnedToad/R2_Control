...
void SPI_SetSpeed(SPI_HandleTypeDef *spiHandle, uint32_t SPI_BaudRatePrescaler); //设置波特率
uint8_t SPI_ReadWriteByte(SPI_HandleTypeDef *hspi, uint8_t TxData);				//SPI收发

void NRF24L01_RX_Mode(SPI_HandleTypeDef *hspi,uint8_t Channel);												  //配置为接收模式
void NRF24L01_TX_Mode(SPI_HandleTypeDef *hspi);												  //配置为发送模式
uint8_t NRF24L01_Write_Buf(SPI_HandleTypeDef *hspi, uint8_t reg, uint8_t *pBuf, uint8_t len); //写数据区
uint8_t NRF24L01_Read_Buf(SPI_HandleTypeDef *hspi, uint8_t reg, uint8_t *pBuf, uint8_t len);  //读数据区
...
uint8_t NRF24L01_TxPacket(SPI_HandleTypeDef *hspi, uint8_t *txbuf);							  //发送一个包的数据
uint8_t NRF24L01_RxPacket(SPI_HandleTypeDef *hspi, uint8_t *rxbuf);							  //接收一个包的数据
#endif
