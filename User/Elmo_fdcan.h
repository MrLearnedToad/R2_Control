#ifndef _ELMO_FDCAN_H_
#define _ELMO_FDCAN_H_
...
// extern uint32_t Elmo_Init(CAN_HandleTypeDef* CANx, TIM_HandleTypeDef * htim);
...
static int Self_test(TIM_HandleTypeDef *htim);
static void Variate_init(void);

/* 数据发送、转换与延时函数,不对外调用 */
// static void Elmo_SendCmd(void);
// static void Elmo_CANSend(CAN_HandleTypeDef* hcan, CANDATA *pCANDATA);
...
/********************新加入的函数******************************/
//__weak void Elmo_CAN_USER_Init(CAN_HandleTypeDef * hcan);
// static uint8_t Elmo_CAN_Send_Data(CAN_HandleTypeDef* hcan, uint8_t *pData, uint16_t ID, uint16_t Len, uint16_t IDE, uint16_t RTR);
static void Queue_Can_Update(CANQUEUE *QUEUE_CAN_IDx);
// void Elmo_SendCmd(CAN_HandleTypeDef* hcan);  //改动，可对外调用
